    ...
    <section className="py-24 bg-slate-900 text-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 space-y-4 md:space-y-0">
          <div>
            <h2 className="text-xs font-bold text-red-500 uppercase tracking-widest mb-3">The Results</h2>
            <p className="text-4xl font-extrabold mb-2">Seeing is Believing</p>
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-px bg-slate-800 rounded-3xl overflow-hidden border border-slate-800 shadow-2xl">
          <div className="relative group overflow-hidden h-[400px]">
            ...
            <div className="absolute top-6 left-6 bg-red-600/90 backdrop-blur-sm text-white px-4 py-1 rounded-lg font-bold text-sm">BEFORE</div>
          </div>
          <div className="relative group overflow-hidden h-[400px]">
            <img src={SAMPLES[activeIdx].after} alt="After" className="w-full h-full object-cover" />
            <div className="absolute top-6 left-6 bg-green-600/90 backdrop-blur-sm text-white px-4 py-1 rounded-lg font-bold text-sm">AFTER</div>
          </div>
        </div>
        ...
        <div className="mt-8 flex justify-center">
          <div className="bg-slate-800/50 backdrop-blur-md px-6 py-4 rounded-2xl border border-slate-700/50 inline-flex items-center space-x-4">
             <span className="text-red-500 font-bold uppercase text-xs tracking-widest">Featured Case:</span>
             <span className="font-bold text-lg">{SAMPLES[activeIdx].title}</span>
